...
class SimulationUtilsTest extends FunSuite with Matchers {
  ...
  * */
  test("Scenario 1 should not be linearizable") {
    ...
    val callGet = ExecutionEvent(1, isCall = true, reqGet, null)
    val returnGet = ExecutionEvent(2, isCall = false, null, resGet)

    val uuidPut = UUID.randomUUID()
    val reqPut = Put("key", "a", uuidPut)
    val resPut = OperationResponse(uuidPut, OpCode.Ok, Some("a"))
    val callPut = ExecutionEvent(3, isCall = true, reqPut, null)
    val returnPut = ExecutionEvent(4, isCall = false, null, resPut)
  ...
  * */
  test("Scenario 2 should be linearizable") {
    ...
    val callPut = ExecutionEvent(1, isCall = true, reqPut, null)
    val returnPut = ExecutionEvent(2, isCall = false, null, resPut)
  ...
  * */
  test("Scenario 3 should be linearizable") {
    ...
    val callPutA = ExecutionEvent(1, isCall = true, reqPutA, null)
    val returnPutA = ExecutionEvent(2, isCall = false, null, resPutA)

    val uuidGetB = UUID.randomUUID()
    val reqGetB = Get("key", uuidGetB)
    val resGetB = OperationResponse(uuidGetB, OpCode.Ok, Some("b"))
    val callGetB = ExecutionEvent(3, isCall = true, reqGetB, null)
    val returnGetB = ExecutionEvent(10, isCall = false, null, resGetB)

    val uuidPutB = UUID.randomUUID()
    val reqPutB = Put("key", "b", uuidPutB)
    val resPutB = OperationResponse(uuidPutB, OpCode.Ok, Some("b"))
    val callPutB = ExecutionEvent(9, isCall = true, reqPutB, null)
    val returnPutB = ExecutionEvent(13, isCall = false, null, resPutB)
  ...
  * */
  test("Scenario 4 should not be linearizable") {
    ...
    val callCasAC = ExecutionEvent(11, isCall = true, reqCasAC, null)
    val returnCasAC = ExecutionEvent(14, isCall = false, null, resCasAC)
